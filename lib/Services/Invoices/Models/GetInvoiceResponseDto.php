...
class GetInvoiceResponseDto
{
     ...
     * @var TotalSummaryDto
     */
    public TotalSummaryDto $total_summary;

    /**
     * @var mixed
     */
    public $reminders_configuration;
    ...
    public function __construct(array $data = [])
    {
        ...
        // Handle single TotalSummaryDto object
        if (isset($data['totalSummary']) && is_array($data['totalSummary'])) {
            $this->total_summary = new TotalSummaryDto($data['totalSummary']);
        } else {
            $this->total_summary = $data['totalSummary'] ?? null;
        }
        $this->reminders_configuration = $data['remindersConfiguration'] ?? null;
    }
    ...
    public function toArray(): array
    {
        ...
        if ($this->total_summary !== null) {
            $result['totalSummary'] = is_object($this->total_summary) && method_exists($this->total_summary, 'toArray') 
                ? $this->total_summary->toArray() 
                : $this->total_summary;
        }
        if ($this->reminders_configuration !== null) {
            $result['remindersConfiguration'] = $this->reminders_configuration;
        }
        return $result;
    }
}
